...
class IncompleteTask(ABC):
    """
    Abstract class for incomplete _tasks, which are further configured
...
class Task(IncompleteTask):
    """
    Basic class for representing _tasks with undefined shared encoder model with config as dict
    """
...
class ClassificationTask(IncompleteTask):
    """
    Class for solving basic classification _tasks with preconfigured converter to features & model head:
...
class Tasks:
    """
    Gathers all _tasks sharing the same base encoder
    """
    ...
    def make_packed_features(
        ...
        """
        Creates features for all _tasks from data field (after preprocessing)

        :param columns: Columns to format in the output to torch type (columns used further for training)
        :param map_kwargs: General arguments for all _tasks in datasets.Dataset.map call (default: batched=True)
    ...
    def make_features(
        ...
        """
        Creates features for all _tasks from data field (after preprocessing) in the format used by MultitaskTrainer
    ...
    def make_model(self, cls: Type[T] = MultitaskModel) -> T:
        """
        Make a multitask model, with forward taking multiple _tasks into account - switching heads

        :param cls: Class with method .create(encoder_path: str, _tasks: Mapping[str, ConfiguredTask])
    ...
    def compute_metrics(self, eval_pred: EvalPrediction) -> Dict[str, float]:
        """
        Compute metrics for all _tasks and join all metrics to single dictionary with suffixes - task-names

        :param eval_pred: Evaluation output: predictions, label_ids
        :return: Joined dictionary for all _tasks "<metric_name> (<task_name>)" -> value: float
        """
        result = {}
